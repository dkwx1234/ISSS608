...
## Loading of packages
The packages required are ggplot2, readr and dplyr. These are all captured in the package *tidyverse*, *dplyr* and the **devtool** *introdataviz*, which we will load using the below script:
...
We also endeavor to understand happiness against household sizes by plotting the happiness against household size.
...
```{r}
ggplot(participants, aes(x = Age_group, y = Joviality, fill = Kids)) +
